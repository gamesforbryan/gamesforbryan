let io = {
	...
	prefix: "ungefubbalblaledspssakwc",
		...
		io.peer.on('open', function(id) {
			...
			io.hostConnection.on('open', function() {
				...
				io.hostConnection.on('data', function(data) {
					// We got data from the host
                    app.acceptDataFromHost(data);
				});
			});
		});
	},
	hostRoom() {
		...
		io.peer.on('open', function(id) {
			...
			io.peer.on('connection', function(conn) {
				...
				if (io.guestConnections.length == 0)
				{
					io.log.push(`Guest '${conn.peer}' connected!`)
					io.guestConnections.push(conn)
					app.onConnectToPeer();
					conn.on('data', function(data) {
						// We got data from a guest
						// Broadcast to everyone else
						io.broadcastMove(data, conn.peer)
						app.acceptDataFromPeer(data, conn.peer)
					});
				}
				else
				{
					io.log.push(`Guest '${conn.peer}' REJECTED!`)
				}
			});
		});
	},
